          ...
          <ul aria-label="Liste des diagnostics" className="space-y-4" role="list">
            {filteredDiagnostics.length === 0 ? (
              ...
              <li key={diag.id} className="bg-white p-4 rounded shadow" role="listitem" aria-label={`Diagnostic du ${diag.date}, problème ${diag.type}, statut ${diag.status}`}>
                <div className="font-semibold" tabIndex={0}>Diagnostic du {diag.date}</div>
                <div className="text-sm text-gray-600" tabIndex={0}>Problème détecté&nbsp;: {diag.type}</div>
                <div className="text-sm" tabIndex={0}>Statut&nbsp;: {diag.status}</div>
                <div className="text-sm mb-2" tabIndex={0}>Description&nbsp;: {diag.description}</div>
                ...
                {activeDiag === diag.id && message && <div className="mt-2 text-green-700" aria-live="polite">{message}</div>}
              </li>
            ))}
          </ul>
        )}
